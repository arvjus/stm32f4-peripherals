...
char buff[100];
uint8_t rx_len = 0, rx_data, rx_completed = 0;
...
int main()
{
	...
	while (1) {
		...
		while (!rx_completed) {
			while(SPI_SendDataIT(&spi2Handle, &data_out, 1) == SPI_BUSY_IN_TX);
			while(SPI_ReceiveDataIT(&spi2Handle, &rx_data, 1) == SPI_BUSY_IN_RX);
		}
